   ...
   while(1) {
       ...
       for (int i = 0; i <= max_fds; i++) {
           if (FD_ISSET(i, &read_fds) != 0) {
               ...
               PacketProtocol *receivedPacket = new PacketProtocol();
               // int bytesReceived = recv(i, buffer, sizeof(buffer), 0);
               int bytesReceived = recv(i, buffer, MAX_PACKET_SIZE, 0);
               ...
               receivedPacket->setPacket(buffer);
               std::string parsedData = receivedPacket->getData();


               // Echo data back to client, right now we just send the string back, but we should be sending a packet_protocol object
